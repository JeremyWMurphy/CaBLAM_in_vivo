...
const uint32_t sampsPerSecond = 2000; // temporal resolution of timer units in seconds * 1/ sampsPerSecond
...
const uint8_t dOutNum = 6; // number if generic digi-out and reward control
...
//-------------------------- Pulse Tracker Data
// n/3: Analog Push next pulse, designated per channel as above, '0' (default) is real-time attibute update, when '1' parameters accumulated in Buffer
...
// s/5: Train start time relative to 'trialTime' (default='-2' disabled)
...
// h/15: toggle a pin
// b/16: Digital Push next pulse, designated per channel as above, '0' (default) is real-time attibute update, when '1' parameters accumulated in Buffer


char knownHeaders[] =    {'a', 'r', 'g', 'n', 'f', 's', 'u', 'y', 'q', 'd', 'p', 'v', 't', 'm', 'l', 'h','b'};
int32_t knownValues[] = { 0, 5000, 0, 0, 0, -2, 0, 6, 25, 90, 10, 0, 0, -2, 0, 0, 0};
int knownCount = 17;
...
bool dOutReceive[dOutNum];
bool dOutReady[dOutNum];
...
//const float setVal = 210.8;  // used to adjust scale
const float setVal = -526.8;
...
//bool hourMicroReset = false;

int32_t elapsedBuff = 0; // ADDED used to do time difference w/o accidental roll over
...
#define syncPin 0   // Trigger other things like a microscope and/or camera
#define syncPinPi 1  // send a session start to a Pi
#define sessionOver 2  // send a session over signal
#define encoderPin A0  // data input for rotation encoder - ADDS 20um PER READ

//  analog input mostly  NOT USED because ~ ADDS 20um PER READ
#define genA0 A1        // ~73um per loop w/ encoder +1 analog reads
// #define genA1 A0     // ~94um per loop w/ encoder +2 analog reads
// #define genA2 A1 //   over run @ 10kHz

#define lickPin 3  // data out for capacitive touch sensor
#define scaleDout 17 // data input from HX711 scale amp
#define scaleSck 16 // serial clock for HX7811 scale amp
//#define rewardPin 32 // connect to servo for reward -depreciated nor dOUtPin 0
const uint8_t dOutPins[] = {4,5,6,7,8,9};  //these are indexed with dOutNum and reward rolled in at ind 0

// interrupt pins TODO - not yet in use, implement frame counter then Flyback
 #define framePin 10   // count scope frames pin assignment
// #define yGalvo  11    // count flyback interrupt pin assignments
...
void setup() {
  ...
  pinMode(dOutPins[4], OUTPUT);
  pinMode(dOutPins[5], OUTPUT);
  ...
  digitalWrite(dOutPins[4], LOW);
  digitalWrite(dOutPins[5], LOW);  
...
//    }   //---- Depreciated
  ...
  if (knownValues[0] == 0) {            // Check to see if Boot state is active, if so do the following
    ...
    if (headerStates[0] == 0) {         // If the boot state isn't registered THINGS TO DO ONCE
      ...
      for (int i=0; i<dacNum; i++) {
        ...
        pulseTrainVars[i][8]=-2;
        trainReceive[i]=0;
        trainReady[i]=0;
        for (int indx = 0; indx<9; indx++) {
          pulseTrainBuffer[i][indx]=-1;          
        }          
      }
  ...
  if (knownValues[0] != 0) {
    ...
    if (headerStates[0] == 1) {  // used to look at delayCount ==0 but changed
     pulseCount = 0; // set frame count to zero to keep track of F's after machine-on
    ...
    */
    updatePulseTracker(pulseTrainVars, pulseTrainBuffer, trainReady); // check buffer for updates to pulse tracker
    updatedOutBuffer(dOutTracker,dOutCounter,dOutBuffer,dOutReady);
...
void genericStateBody() {
  ...
  // checkDOut(); // can't be here, would cause base-state triggering/channel reset
  ...
  updateOutSignals();                        //this encodes a report of  DAC out, and dOut values          
   ...
   if (scale.is_ready()) {                              // 
    scaleVal = int(round(scale.get_units(1)*100)); // convert float to int with hundreths info preserved
...
void incrementTime() {
  ...
  if (!ranTwice) {      //time synced and time is zero
    ...
    if ((trialTime - (360000000 * rolledHours) + microsPerLoop) < 360000000) {   // while accumulator is less than an hour
  
      // Check that time keeping incremented by the loop is accurate to 10kHz (less than 100us drift)
      if ( abs( ((int32_t)microsPerLoop - abs((int32_t)loopedTime - (int32_t)trialTime ) ) <= 10 )) {
      //if ( abs( loopedTime - trialTime ) <= 10 ) {
      //if (true) {
       ...
       underRun = true;
        ...
        //trialTime = trialTime + microsPerLoop; //these are for debugging 
        //dataReport();
      }
      ...
      //loopTime = 0;
      //hourMicroReset = true; // set loopTime reset on next loop
  
      // Check that time keeping incremented by the loop is accurate to 10kHz (less than 100us drift)
      if ((abs( (int32_t)microsPerLoop - (int32_t)loopedTime - (int32_t)trialTime ) ) <= 10)  {
      //if ( abs( loopedTime - trialTime ) <= 10 ) {
        ...
        underRun = true;
        Serial.println("Clock under run -> kill");
        // rolledHours++;   //these are for debugging 
        //trialTime = trialTime + microsPerLoop;
        // dataReport();
      }
      ...
      if (abs((int32_t)microsPerLoop - abs((int32_t)loopedTime - (int32_t)trialTime ) ) <= 10 ) {
        ...
        underRun = true;
        Serial.println("Clock under run -> kill");
        // Serial.println("5hr rollover attempt");
        // dataReport();
      }
  ...
  while (Serial.available() > 0 && newData == 0) {    // set a loop to run if there is a bit in the buffer until the message is read
    ...
    if (recvInProgress == false) {                // If this is a new message (variable; not partially read)
      ...
      if (rc == endMarker ) {                 // if the bit is a revieve message stop 
        ...
        || (selectedVar == 10) || (selectedVar == 11) || (selectedVar == 12) || (selectedVar == 13) || (selectedVar == 16)) {   
        ...
        // if Output channel recvd'....
          ...
          valChar[vDex]='\0';
           ...
           selectedChan = uint8_t(String(chanChar).toInt());  //convert the Char to string and then to integer and call it schan
          ...
          if ((selectedVar == 1) || (selectedVar == 2) || (selectedVar == 16)) {
            setDOutTracker(selectedVar, selectedChan-1, valAr[selectedVar],dOutTracker,dOutCounter,dOutStart,dOutReceive,dOutReady,dOutBuffer);
            
          }
          else {
            setPulseTrainVars(selectedVar, valAr[selectedVar],trainReceive, trainReady, selectedChan, pulseTrainBuffer); //add update bool
          }
...
void setPulseTrainVars(int recVar, int parsedValue, bool tRec[], bool tRed[], uint8_t parsedChan, int32_t pBuffer[][9]) {
  // depreciated
  ...
  if (recVar == 3) {
    if ((tRec[parsedChan - 1] == true) && (parsedValue == 0)) { // check to see if buffer is buffer data is ready
      ...
      tRed[parsedChan - 1] = false;       // make sure half-set doesn't fire if re-opened
      //Serial.println("open");
    }
    else if (parsedValue == -1) { //  treat as erase 
      tRec[parsedChan - 1] = false;       // reset buffer routing
      tRed[parsedChan - 1] = false;       // make sure any transmitted doesn't fire if being erased
      for (int idx = 0; idx < 9; idx++) {                             // loop through buffer parameters
        if (pBuffer[parsedChan - 1][idx] != -1) {                                  // if not default value
          pBuffer[parsedChan - 1][idx] = -1;                                       // set default value
          }
        }
    }
  }
...
void updatePulseTracker(int32_t pVars[][13], int32_t pBuffer[][9], bool tReady[]) {
  ...
  for ( int i = 0; i < dacNum; i++) {                                   // Loop through Dacs/ lines of tracker
      if ( (tReady[i] == true) && ((trialTime >= pBuffer[i][6]) && (pBuffer[i][6] !=-1)) && (pVars[i][1]==1) ) { // if pulse vars ready + the channel is not active or its time for the new train
        //Serial.print("Updating Channel ");
        //Serial.println(i);
//        Serial.print(", Trial time, ");
//        Serial.print(trialTime);
//        Serial.print(", Buffer Time , ");
//        Serial.print(pBuffer[i][6]);
//        Serial.print(", chan latch , ");
//        Serial.print(pVars[i][1]);
        for (int idx = 0; idx < 9; idx++) {                             // loop through buffer parameters
          if (pBuffer[i][idx] != -1) {                                  // if not default value
            pVars[i][varMap[idx]] = pBuffer[i][idx];                    // copy new parameter to pulse train vars
//            Serial.print("Updating Parameter ");
//            Serial.print(varMap[idx]);
//            Serial.print(" to ");
//            Serial.println(pBuffer[i][idx]);
            pBuffer[i][idx] = -1;                                       // set default value
          }
          else if ((idx==4) && (pBuffer[i][idx] == -1)) {
            // make sure puse count always reset when nothing there to replace
            pVars[i][varMap[idx]] = -2;
...
void dataReport() {
  ...
  //Serial.print(loopTime); //loopTime
  //Serial.print('0');
  Serial.print(',');
  Serial.print(knownValues[14]);  //load cell
  //Serial.print(rolledHours); //rolledHours
  Serial.print(',');
  Serial.print(lickSensorValue); // lick sensor
  //Serial.print(loopedTime); //loopedTime
  Serial.print(',');
  Serial.print(encoderAngle);     //rotary encoder value
  //Serial.print(hourMicroReset);  //hourMicroReset
  Serial.print(',');
...
void setDOutTracker(int varType, uint8_t varChan, int32_t varVal, bool outTracking[][2], 
                        uint32_t outCounting[][2], int32_t dOutSt[],bool dOutRcv[],bool dOutRdy[],int32_t dOutBuff[][2]) {

  // Find whether buffer process is being controlled and flip the bit
  if (varType == 16) {
    if ((dOutRcv[varChan] == true) && (varVal == 0)) { // check to see if buffer is buffer data is ready
      dOutRdy[varChan] = true;       // if so, flag channel ready
      dOutRcv[varChan] = false;       // reset buffer routing
      //Serial.println("close");
    }
    else if (varVal == 1) {
      dOutRcv[varChan] = true;       // start buffer routing
      dOutRdy[varChan] = false;       // make sure half-set doesn't fire if re-opened
      //Serial.println("open");
    }
    else if (varVal == -1) { //  treat as erase 
      dOutRcv[varChan] = false;       // reset buffer routing
      dOutRdy[varChan] = false;       // make sure any transmitted doesn't fire if being erased
      for (int idx = 0; idx < 2; idx++) {                             // loop through buffer parameters
        if (dOutBuff[varChan][idx] != -2) {                                  // if not default value
          dOutBuff[varChan][idx] = -2;                                       // set default value
          }
        }
    }
  }
  if (dOutRcv[varChan] == true) {       // if buffer routing
    // Buffering can only include a start time and duration corresponds to dOutBuff 0 and 1

    // START TIME
    if (varType == 2) {
      dOutBuff[varChan][1] = varVal;
    }
    //  DURATION
    else if (varType == 1) {
      dOutBuff[varChan][0] = varVal;
    }
    
  }
  else if (dOutRcv[varChan] == false) {
...
// ******************************************************
void updatedOutBuffer(bool outTracking[][2],uint32_t outCounting[][2] , int32_t dOutBuff[][2], bool dOutRdy[]) {
  int strtVr = 2;
  int durVr = 1;
  int32_t strtRst = -2;
  
  for ( uint8_t i = 0; i < dOutNum; i++) {                                   // Loop through Dacs/ lines of tracker
      if ( (dOutRdy[i] == true) && ((trialTime >= dOutBuff[i][1]) && (dOutBuff[i][1] >=0))) { // if pulse vars ready + the channel is not active or its time for the new train
          int32_t strtTime;
          int32_t durTime;
        
          strtTime = dOutBuff[i][1];                    // copy new parameter to pulse train vars
          // clear last thing in channel
          setDOutTracker(strtVr,i, strtRst,dOutTracker,dOutCounter,dOutStart,dOutReceive,dOutReady,dOutBuffer);
          checkDOut();
          // then just update the variables
          setDOutTracker(strtVr,i, strtTime,dOutTracker,dOutCounter,dOutStart,dOutReceive,dOutReady,dOutBuffer);

          if (dOutBuff[i][0] != -2){
            durTime = dOutBuff[i][0];
            setDOutTracker(durVr,i, durTime,dOutTracker,dOutCounter,dOutStart,dOutReceive,dOutReady,dOutBuffer);            
          }
          
          dOutBuff[i][0] = -2;                                       // set default value
          dOutBuff[i][1] = -2;
          dOutRdy[i] = false;                                              // reset redy bool when buffer flushed
      }  
  }
}
// ******************************************************
...
void sendDOut(int outChan, bool startR) {  //evaluates digital out timine and sets pin no need to loop here (done by caller)
      if (startR==true) {  //if start signal trig sent set tracker/counter and send pin high
      ...
      else if ((dOutTracker[outChan][1] == 1) && (trialTime >= dOutCounter[outChan][0])) { // if duration completed
...
void writeAnalogOutValues(uint32_t dacVals[]) {
  mcp.setChannelValue(MCP4728_CHANNEL_A, dacVals[0]);
  mcp.setChannelValue(MCP4728_CHANNEL_B, dacVals[1]);
  mcp.setChannelValue(MCP4728_CHANNEL_C, dacVals[2]);
  mcp.setChannelValue(MCP4728_CHANNEL_D, dacVals[3]);
  ...
  if (pulseTracker[i][10] != -2) {   // If the channel is not disabled  - evaluate active state, otherwise leave off - state fire flips bit in the header so not tested here
    ...
    if (pulseTracker[i][1]==1) {         // If the channel is off - evaluate start conditions and if pulsing initiated start train timer
      if (pulseTracker[i][10] <= trialTime) {   // if start time met - evaluate fire channel
        ...
        else if (pulseTracker[i][8] > 0) { // changed from, count reset added ((pulseTracker[i][11] == 0) && (pulseTracker[i][8] > 0)) {
    ...
    if (pulseTracker[i][1]==0) { // If the channel is active - evaluate turning it off by duration and by pulse count, if the channel shouldn't turn off - pulse tracking
      ...
      if (((pulseTracker[i][10] + pulseTracker[i][11]) <= trialTime) && ((pulseTracker[i][8] < 1) || (pulseTracker[i][8] == -2))) { 
        ...
        pulseTracker[i][8] = -2; // reset pulse count
      }
      // if we run out of pulses - flip off channel, flip start -2
        ...
        if (stimType == 0) {                          // If (main) the channel is set to a square wave
         ...
         if (pulseState == 0) {                   // changed from else if Else (second, not in pulse phase), if not in pulse phase 
       ...
       // 2) Asymm Cosine
        ...
        if (pulseState == 0) { // changed from else if
      ...
      if (pulseTracker[i][1]==0) {
        // if the channel was active and kill was flipped, reset pulse count else could bug other signals
        pulseTracker[i][8] = -2; // reset pulse count
